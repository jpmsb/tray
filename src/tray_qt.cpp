...
namespace tray_qt {
   ...
   *
   * Prefer notification_cb; otherwise use the first named action when present.
   *
  ...
  void notify(struct tray *tray) {
    ...
    if (state().trayMenu != nullptr && QtTrayMenu::supportsMessages()) {
      auto *callback = resolve_notification_callback(tray);
      if (tray->notification_icon != nullptr) {
        state().trayMenu->showMessage(tray->notification_title, tray->notification_text, tray->notification_icon, callback);
      } else {
        state().trayMenu->showMessage(tray->notification_title, tray->notification_text, callback);
      }
    }
  }
